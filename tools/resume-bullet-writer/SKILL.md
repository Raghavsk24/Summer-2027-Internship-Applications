...
**Rule: Keep bullets to 1-2 lines maximum, and make the last line 90-100% full (see Line-Fill Rule below)**
...
If too long, prioritize most impressive metrics, then check line fill (see Line-Fill Rule below):
...
## Line-Fill Rule (house style, mandatory)

The LAST line of every resume bullet must fill 90-100% of the line width. No short, half-empty trailing lines: a bullet that wraps by a word or two looks unfinished, and a one-line bullet should also reach about 90% or more.

How to fix a bullet that is too short or wraps:
1. **Cut first.** Drop filler and secondary detail (e.g. "user-submitted", "from public sites", "on the extracted features"). Trimming is easier than padding and keeps bullets tight.
2. **Add only when cutting would lose a key fact or metric**, and only real, verified detail. Never pad with filler or invent facts.
3. **Move a fact** (an award, a secondary metric) to a neighboring bullet if it does not fit. Never squeeze a line past 100%.

Verify by measuring, not by eye. Compile to a scratch folder (so no build files land in the repo), then from the repo root (C:/Users/senth/Resume) run `python tools/check_line_fill.py <resume.pdf>`. Add an entry name (e.g. `clusion`) to check just that entry. Every bullet must report 90-100%. Re-run after every edit, because one changed word can re-wrap a line. Bold, italic and underlined text is wider than plain text, so character counts mislead.
...
- ✅ 1-2 lines maximum, last line 90-100% full
...
5. ✅ Test length → Keep to 1-2 lines with the last line 90-100% full (measure it, don't eyeball)
